...
/*void afficherPageModificationPatient() {
  ...
  nomASearch.trim();  // Supprimer les espaces en début et fin
  ...
  while (file.available()) {
    ...
    if (ligne.startsWith("Nom: ") && ligne.indexOf(nomASearch) != -1) {
      patientTrouve = true;
      nom = ligne.substring(5);  // Extraire le nom du patient
      ligne = file.readStringUntil('\n');  // Lire le téléphone
      telephone = ligne.substring(13);    // Extraire le téléphone
      ligne = file.readStringUntil('\n');  // Lire le numéro de sécurité sociale
      numSecu = ligne.substring(26);      // Extraire le numéro de sécurité sociale
      ligne = file.readStringUntil('\n');  // Lire le groupe sanguin
      gs = ligne.substring(14);           // Extraire le groupe sanguin
      break;
    }
  }
...
/void modifierPatient() {
  ...
  while (file.available()) {
    ...
    if (ligne.startsWith("Nom: ") && ligne.indexOf(nom) != -1) {
      patientTrouve = true;
  ...
  if (newFile) {
    ...
    if (patientTrouve) {
      server.send(200, "text/html", "<h3 style='color:green;text-align:center;'>✅ Informations modifiées avec succès !</h3>");
    } else {
      server.send(404, "text/html", "<h3 style='color:red;text-align:center;'>⚠️ Patient non trouvé !</h3>");
    }
  } else {
...
void setup() {
  ...
  server.on("/modifier_patient", HTTP_GET, afficherPageModificationPatient);
