...
void supprimerHistorique() {
  ...
  // ⚠️ Formatage complet du SPIFFS (Supprime tous les fichiers)
...
void setup() {
  ...
  server.on("/supprimer_patients", HTTP_GET, afficherPageSuppressionPatient);  // Fonction afficherPageSuppressionPatient()
...
void loop() {
  server.handleClient();
    delay(100);  // Attends 10 secondes avant de réessayer
